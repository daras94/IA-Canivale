...
(define (sucesores num_op nodo_actual nivel)
  (let* (;Declaramos las variables para las operacionesde trasicion que son posibles.
         (op_ida (list (list 0 1 1  0 -1)  ; o1 = 1 canibal se monta en la barca.  van a la orilla destino
         ...
         (op_vuelta (list (list 0 -1 0  0 1)  ; o6 = 1 canibal se monta en la barca.  Van a la orilla origen
         ...
         (op_transicion (list null ))
         (new_sucesor   (list null ))
        )
        ;Declaramos las expresiones a usar.
        (and (if (equal? 0 (modulo nivel 2)) (set! op_transicion op_vuelta) (set! op_transicion op_ida))
             (cond
...
(define (MC1)
  (let* ( ; Declaracion de variables.
         ...
         (abiertos  (list (list (list 0 0 0 3 3) 0)) )  ; Lista de abiertos (My, Cy, B, Mx, Cx) y su .
         (meta      (list 3 3 1 0 0)                 )  ; Definimos el estao meta del problema.
        )
    ...
    (while (and (not (equal? meta (car actual))) (not (empty? abiertos)))  ; Bucle de busqueda en el arbol.
        ...
        (cond
          ...
          [else
             ...
             (printf "/**********************************************************************/\n")
             (printf " Actuales: ~v \n" actual)
             (set! sucesor  (reverse (map (lambda (s1) (fun s1 (length actual))) (sucesores 0 (car actual) (length actual)))))
             (printf " Sucesores: ~v \n" sucesor)
